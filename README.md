...
The seed creates a synthetic scenario so the dashboard has data on first boot. To populate
Caliber with **real** model predictions instead, `tools/ingest_real.py` trains two genuine
classifiers on a scikit-learn dataset and posts their measured outputs to the API:

- production model: a calibrated logistic regression (well-calibrated)
- candidate model: Gaussian naive Bayes (naturally over-confident)

```bash
pip install -r tools/requirements.txt
python tools/ingest_real.py --dataset digits          # against a local API
API_URL=https://caliber-api.onrender.com python tools/ingest_real.py --dataset digits   # against the deployed API
```

This registers a `digits-classifier` model with two real versions and streams ~719 real
`(confidence, predicted_label, ground_truth)` tuples through `/api/predictions`. The resulting
calibration is measured, not seeded — the calibrated model reports ECE around 0.08 while the
over-confident one reports around 0.17, and low-confidence predictions are routed to review. The
schema and metric code are unchanged; only the data source is swapped, which is the point: Caliber
is model-agnostic, so the same endpoints accept any real classifier's inference stream.
